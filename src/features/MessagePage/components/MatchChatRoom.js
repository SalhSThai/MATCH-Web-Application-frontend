...
export default function MatchChatRoom(props) {
  ...
  return (
    <div className="relative w-full h-[100px] bg-white  flex overflow-hidden  ">
      ...
      <div role="button" className="h-full grow flex flex-col gap-2 " onClick={openChat}>
        ...
        <div className="w-full  h-[50%]  flex flex-col  items-center">
          <p className="text-sm font-normal grow px-5 self-start">{` ${rawInfo?.ChatMessages?.[0]?.message ?? ''
            }`}</p>{' '}
          <div className="place-self-end text-sm text-right pr-5">{`${rawInfo?.ChatMessages?.[0]?.createdAt.slice(11, 16) ?? ''
            }`}</div>
        </div>
      </div>
